...
from settings import API_IP, API_PORT, API_IP_IMG_REC, API_PORT_IMG_REC
        ...
        while True:
            ...
            if message['cat'] == "control":
                if message['value'] == "start":
                    ...
                    self.clear_queues()
                    self.command_queue.put("FW000") # ack_count = 1
                    
                    # Small object direction detection
                    #self.command_queue.put("FC010")
#                     if count == 0:
#                         self.command_queue.put("FC010")
#                         count +=1
#                         continue
...
# 
#                     elif self.small_direction == None or self.small_direction == 'NA':
            ...
            if message.startswith("ACK"):
                ...
                if self.ack_count == 2:
                    try:
                        ...
                        #if results['image_id'] == 39:
                        if self.small_direction == "Left Arrow": 
                            self.command_queue.put("FC020") # ack_count = 5
                            self.command_queue.put("FC040")
                        elif self.small_direction == "Right Arrow":
                            self.command_queue.put("FC030") # ack_count = 5
                            self.command_queue.put("FC040") # ack_count = 5
                        else:
                            self.command_queue.put("FC020") # ack_count = 5
                            self.command_queue.put("FC040")
                ...
                if self.ack_count == 4:
                    try:
                        self.logger.debug("First ACK received, robot reached second obstacle!")
                        self.small_direction = self.snap_and_rec("Large")
                        #if results['image_id'] == 39:
                        if self.small_direction == "Left Arrow": 
                            self.command_queue.put("FC300") # ack_count = 5
                            self.command_queue.put("FC310") # ack_count = 5
                            self.command_queue.put("FC380") # ack_count = 5
                            self.command_queue.put("FC400") # ack_count = 5
                            self.command_queue.put("DT100") # ack_count = 5
                            #self.command_queue.put("FC400")
                            #self.command_queue.put("FC500")
                            #self.command_queue.put("DT200")
                        elif self.small_direction == "Right Arrow":
                            self.command_queue.put("FC320") # ack_count = 5
                            self.command_queue.put("FC330")
                            self.command_queue.put("FC380") # ack_count = 5
                            self.command_queue.put("FC410") # ack_count = 5
                            self.command_queue.put("DT100") # ack_count = 5
                            #self.command_queue.put("DT200")
                        else:
                            self.command_queue.put("FC320") # ack_count = 5
                            ...
                            self.command_queue.put("FC380") # ack_count = 5
                            self.command_queue.put("FC410") # ack_count = 5
                            self.command_queue.put("DT100")
#                             self.command_queue.put("FC300") # ack_count = 5
#                             self.command_queue.put("FC310") # ack_count = 5
#                             self.command_queue.put("FC380") # ack_count = 5
#                             self.command_queue.put("FC400") # ack_count = 5
#                             self.command_queue.put("DT100") # ack_count = 5
                            self.logger.debug("Failed first one, going right by default!")
                            #self.logger.debug("Failed first one, going left by default!")
                    except:
                        self.logger.info("No need to release near_flag")
    ...
    def command_follower(self) -> None:
        while True:
            ...
            stm32_prefixes = ("STOP", "ZZ", "UL", "UR", "PL", "PR", "RS", "OB", "FC", "DT", "FW")
...
#
    def snap_and_rec(self, obstacle_id_with_signal:str) -> None:
        ...
        #retry_count = 0
        while True:
            # print(obstacle_id_with_signal + "hello")
            obstacle_id = obstacle_id_with_signal
            #obstacle_id, signal = obstacle_id_with_signal.split("_") # eg Small_Near?
            print("Capturing image for obstacle id:" + obstacle_id)
            url = f"http://{API_IP_IMG_REC}:{API_PORT_IMG_REC}/image"
            filename = f"{int(time.time())}_{obstacle_id}.jpg" #_{signal}
                ...
                while not detected:
                    ...
                    with open(filename, 'rb') as file:
                        ...
                        if response.status_code == 200:
                            ...
                            image_id_value = SYMBOL_MAP.get(results['image_id'])
                            ...
                            self.logger.info(f"Image recognition results: {results} ({image_id_value})")
                            
                            
                            if image_id_value != 'NA' or iteration > 2:
                                detected = True
                                
                                #self.android_queue.put(AndroidMessage("cat": image-rec, "value": {"image_id": image_id_valuxe, "obstacle_id": obstacle_id}))
                                #self.movement_lock.release()
                                self.android_queue.put(AndroidMessage("image-rec", results))
                                return image_id_value
                        
                            else:
                                self.logger.info("Result NA, retrying")
    ...
    def request_stitch(self):
        url = f"http://{API_IP_IMG_REC}:{API_PORT_IMG_REC}/stitch"
    ...
    def check_api(self) -> bool:
        url = f"http://{API_IP_IMG_REC}:{API_PORT_IMG_REC}/status"
        try:
