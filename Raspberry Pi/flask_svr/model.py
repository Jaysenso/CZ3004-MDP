...
def load_model():
    ...
    # model = torch.hub.load('./', 'custom', path='Wk8_finetuned_best.pt', source='local', force_reload=True)
    # model = torch.hub.load('ultralytics/yolov5', 'custom', path='Wk8_finetuned_best.pt',`` force_reload=True,trust_repo=True)
    # model = torch.hub.load('ultralytics/yolov5', 'custom', path='Week_8.pt', force_reload=True,trust_repo=True)
    model = torch.hub.load('ultralytics_yolov5_master', 'custom', path='Week_8_v2.pt', force_reload=True,trust_repo=True, source='local')
    return model
...
def draw_own_bbox(img,x1,y1,x2,y2,label,color=(36,255,12),text_color=(0,0,0)):
    ...
    cv2.imwrite(f"runs/own_results/raw_image_{label}_{rand}.jpg", img)
    ...
    cv2.imwrite(f"runs/own_results/annotated_image_{label}_{rand}.jpg", img)
    ...
    try:
        ...
        if len(pred_list) > 1:
            ...
            for _, row in pred_list.iterrows():
                if row['name'] != 'Bullseye' and row['confidence'] > 0.2 and ((current_area * 0.8 <= row['bboxArea']) or (row['name'] == 'One' and current_area * 0.6 <= row['bboxArea'])):
...
def predict_image_week_9(image, model):
    ...
    if pred_list.size != 0:
        ...
        for _, row in pred_list.iterrows():
            if row['name'] != 'Bullseye' and row['confidence'] > 0.2:
                pred = row    
                break
...
def stitch_image():
    ...
    imgFolder = 'runs'
    stitchedPath = os.path.join(imgFolder+"/stitched/", f'stitched-{int(time.time())}.jpeg')
    ...
    # Get the width and height of each images
