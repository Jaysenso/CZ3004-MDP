...
CmdConfig cfgs[19] = {
	...
	{1800, 700, 115 ,-88.5, DIR_FORWARD}, // FR00
	{500, 1700, 50, -90, DIR_BACKWARD}, // BL00
	{1800, 500, 115, 90, DIR_BACKWARD}, // BR00,

	{700, 1800, 50, 89, DIR_FORWARD}, // FL20
	{1800, 700, 115 ,-88.5, DIR_FORWARD}, // FR20
	{500, 1700, 50, -90, DIR_BACKWARD}, // BL20  OUTDOOR
	{1800, 500, 115, 89.5, DIR_BACKWARD}, // BR20,
...
//	if (aRxBuffer[2] >= '0' && aRxBuffer[2] <= '9') val += (aRxBuffer[4] - 48) * 100;

	if (aRxBuffer[2] >= '0' && aRxBuffer[2] <= '9') {
		val += (aRxBuffer[2] - '0') * 100; // Hundreds place
	}

	if (aRxBuffer[3] >= '0' && aRxBuffer[3] <= '9') {
		val += (aRxBuffer[3] - '0') * 10; // Tens place
	}

	if (aRxBuffer[4] >= '0' && aRxBuffer[4] <= '9') {
		val += (aRxBuffer[4] - '0'); // Ones place
	}
...
//		__ADD_COMMAND(cQueue, 0, 0); // stop
	...
	else if (aRxBuffer[0] == 'A')
	{
		...
		if (aRxBuffer[1] >= '0' && aRxBuffer[1] <= '3'){
			val += (aRxBuffer[1] - '0') * 100; // Hundreds place
		}

		if (aRxBuffer[2] >= '0' && aRxBuffer[2] <= '9'){
					val += (aRxBuffer[2] - '0') * 10; // Tens place
		}

		if (aRxBuffer[3] >= '0' && aRxBuffer[3] <= '9') {
				val += (aRxBuffer[3] - '0'); // Ones place
		}
...
void runOledTask(void *argument)
{
  ...
  for(;;)
  {
	snprintf(ch, sizeof(ch), "%u", curCmd.val);
	...
	OLED_ShowString(0, 30, (char *) aRxBuffer);
